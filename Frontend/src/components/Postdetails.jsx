...
const Postdetails = () => {
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:3000/d/postdetails?id=${id}`);
        if (!response.ok) {
          throw new Error("Failed to fetch service details");
        }
        const data = await response.json();
  ...
  return (
    <div className="w-[1400px]">
      <div className="container mx-auto mt-10 flex justify-center px-[10%] flex-col gap-y-3">
        <div className="bg-white shadow-lg rounded-lg p-6 min-w-[900px] w-full flex flex-col">
          <img src={service.image} alt={service.title} className="w-full h-[400px] rounded-lg mt-4" />
          <h1 className="text-3xl font-bold">{service.title}</h1>
          <p className="mt-4">{service.description}</p>
          <p className="mt-4">Price: Rs {service.price}</p>
          <p className="mt-4">Category: {service.category}</p>
        ...
        <div className="flex space-x-4 mt-6 w-full mb-20">
          <button onClick={handleBookNowClick} className="flex items-center bg-purple-500 text-white rounded-lg p-2 hover:bg-purple-600 transition">
            Book Now
          </button>
          <button className="flex items-center bg-blue-500 text-white rounded-lg p-2 hover:bg-blue-600 transition">
            <FaCommentAlt className="mr-2" /> Chat
        ...
        <Modal isOpen={isModalOpen} onRequestClose={handleCloseModal}>
            ...
            {[...Array(getDaysInCurrentMonth())].map((_, index) => {
              ...
              return (
                <button
                  ...
                  className={`w-full h-[50px] ${isBooked ? 'bg-red-500' : isSelected ? 'bg-yellow-500' : 'bg-green-500'} text-white rounded`}
                >
                  {day}
                </button>
              );
          ...
          {selectedDate && (
            <div className="mt-4">
              <button onClick={handleConfirmBooking} className="bg-orange-500 text-white p-2 rounded">Pay Now</button>
            </div>
          )}

          <button onClick={handleCloseModal} className="mt-4 bg-gray-500 text-white p-2 rounded">Close</button>
        </Modal>
      </div>
    </div>
  );
